...
const getUserPlaylists = asyncHandler(async (req, res) => {
  ...
  const userPlaylists = await Playlist.find({ owner: userId }).select(
    "-videos"
  );
  if (!userPlaylists || userPlaylists.length === 0)
    throw new ApiError(
      500,
      "Something went wrong while finding user playlists."
    );
...
const addVideoToPlaylist = asyncHandler(async (req, res) => {
  ...
  if (playlist?.owner.toString() !== req.user?._id.toString())
    throw new ApiError(
      401,
      "User is not authenticate to add video in the playlist."
    );
  ...
  if (isVideoPresent)
    throw new ApiError(409, "Video already present in the playlist.");
  else playlist.videos.push(new mongoose.Types.ObjectId(videoId));
...
const removeVideoFromPlaylist = asyncHandler(async (req, res) => {
  ...
  if (playlist?.owner.toString() !== req.user?._id.toString())
    throw new ApiError(
      401,
      "User is not authenticate to delete video in the playlist."
    );
...
const updatePlaylist = asyncHandler(async (req, res) => {
  ...
  if (!mongoose.isValidObjectId(playlistId))
    throw new ApiError(400, "Invalid Playlist Id.");
  if (!tittle) throw new ApiError(400, "tittle is required.");
  if (!description) throw new ApiError(400, "description is required.");
