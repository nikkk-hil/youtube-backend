...
const publishVideo = asyncHandler(async (req, res) => {
  ...
  if (!thumbnailPath) throw new ApiError(400, "Thumbail is required!!");
  ...
  if (!video)
    throw new ApiError(
      500,
      "Something went wrong while creating video database"
    );
...
const getVideoById = asyncHandler(async (req, res) => {
  ...
  if (!videoId) throw new ApiError(400, "VideoId is mising in the url");
  ...
  const video = await Video.findById(videoId).populate(
    "owner",
    "username avatar"
  ); // add username and avatar to the owner object
  ...
  const isSubscribed = subscription.length === 0 ? false : true;
  const isLiked = likedByUser.length === 0 ? false : true;
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        { video, isSubscribed, likes, isLiked },
...
const updateVideo = asyncHandler(async (req, res) => {
  ...
  if (!video) throw new ApiError(404, "Video File is not Found!!");
...
const deleteVideo = asyncHandler(async (req, res) => {
  ...
  if (!video)
    throw new ApiError(
      500,
      "Something went wrong while extracting video document from database"
    );
...
const togglePublishStatus = asyncHandler(async (req, res) => {
  ...
  if (video.owner.toString() !== req.user_id.toString())
    throw new ApiError(
      403,
      "You are not authozied to toggle publish status of video!!"
    );
